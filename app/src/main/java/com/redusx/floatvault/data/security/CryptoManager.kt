...
class CryptoManager @Inject constructor(
    ...
    companion object {
        private const val DB_PREFS_NAME = "floatvault_db_security"
        private const val LEGACY_DB_PREFS_NAME = "datamanager_db_security"
    ...
    fun getOrCreateDbPassphrase(context: Context): ByteArray {
        ...
        var encryptedPassphraseB64 = prefs.getString(KEY_ENCRYPTED_DB_PASSPHRASE, null)

        if (encryptedPassphraseB64 == null) {
            val legacyPrefs = context.getSharedPreferences(LEGACY_DB_PREFS_NAME, Context.MODE_PRIVATE)
            val legacyPassphrase = legacyPrefs.getString(KEY_ENCRYPTED_DB_PASSPHRASE, null)
            if (legacyPassphrase != null) {
                encryptedPassphraseB64 = legacyPassphrase
                prefs.edit().putString(KEY_ENCRYPTED_DB_PASSPHRASE, legacyPassphrase).commit()
            }
        }
    ...
    fun clearDbPassphrase(context: Context) {
        ...
        context.getSharedPreferences(LEGACY_DB_PREFS_NAME, Context.MODE_PRIVATE).edit().clear().commit()
    }
